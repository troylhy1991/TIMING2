...
class TIMING_Nanowell(QFrame):
	def __init__(self, dataPath, BID, row, col, t, t_span, tag):
	...
	def setupUI(self):
		...
		self.setWindowTitle("TPS 1.8 Nanowell")
		self.show()
		
		
	def getImagePath(self, CH):
		path = self.path + "B" + str(self.BID).zfill(3) + "/crops_8bit_s/imgR"+str(self.row)+"C"+str(self.col)+"CH"+str(CH)+"/imgR"+str(self.row)+"C"+str(self.col)+"CH"+str(CH)+"_t"+str(self.t)+".tif"
		return path
	
	def getBGImagePath(self,CH):
		path = self.path + "B" + str(self.BID).zfill(3) + "/crops_8bit_s/imgR"+str(self.row)+"C"+str(self.col)+"CH"+str(CH)+"bg/imgR"+str(self.row)+"C"+str(self.col)+"CH"+str(CH)+"bg_t"+str(self.t)+".tif"
		return path
	...
	def starButtonEvent(self):
		...
		self.monoWell.append(TIMING_Feature_Board(self.path, self.BID, self.row, self.col, self.E, self.T, self.Tag, 1, self.t_span))
	...
	def sampleButtonEvent(self):
		...
		self.sampler = TIMING_DT_Sampler(self.path, self.t_span, self.Tag, self.E, self.T)
	
	def editButtonEvent(self):
		
		self.ReEdit = TIMING_ReEditBox(self.path, self.BID, self.row, self.col, 1, self.t_span, self.Tag)
	...
	def load_well_data(self):
		...
		path = path + "features/2_Cell_Pool/" + "B" + str(B).zfill(3) + "R" + str(R) + "C" + str(C)
		...
		for j in range(1,5):
			...
			if os.path.isfile(fname):
				...
				for i in range(1,6):
					x = temp[i].split()
					x = [x[i] for i in range(3,3+self.t_span)]
...
def main():
	...
	t = 1
	t_span = 80
	cube = TIMING_Nanowell(path, BID, row, col, t, t_span)
	sys.exit(app.exec_())
